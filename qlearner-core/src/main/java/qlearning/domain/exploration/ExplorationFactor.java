...
/**
 * Represent's an agent's likelihood of taking non-optimal actions in order to explore the problem
 * space.
 * 
 * <p>
 * A value of 0.0 means that the agent should never explore, and a value of 1.0 means that the agent
 * will always explore. How exactly your agent explores is decided by your
 * {@link ExplorationStrategy}.
 * </p>
 */
@Immutable
@ThreadSafe
public final class ExplorationFactor extends Number {
  ...
  /**
   * An {@code ExplorationFactor} whose {@link #shouldExplore(Number)} will always return
   * {@code true}.
   */
  public static final ExplorationFactor ALWAYS_EXPLORE =
      new ExplorationFactor(ALWAYS_EXPLORE_VALUE);
   ...
   * 
   * @param value the value of this factor in the interval [0,1]. A value of 0.0 means that the
   *        agent should never explore, and a value of 1.0 means that the agent will always explore.
   */
   ...
   * 
   * @param checkValue the value to compare against this factor in the interval [0,1).
   * @return {@code true} if exploration should take place, {@code false} otherwise.
   */
  public boolean shouldExplore(@Nonnegative @CheckForSigned Number checkValue) {
    ...
    if (doubleCheckValue < 0.0 || doubleCheckValue >= 1.0) {
      throw new IllegalArgumentException(
          "Test value must be within [0,1). Given value: " + checkValue);
    }
