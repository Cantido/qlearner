 ...
 * <p>
 * A factor of 0 will make the agent not learn anything, while a factor of 1 would make the agent
 * consider only the most recent information. In fully deterministic environments, a learning rate
 * of 1 is optimal. When the problem is stochastic, the algorithm still converges under some
 * technical conditions on the learning rate, that require it to decrease to zero.
 * </p>
 * This value needs to be chosen in correlation with how often a given {@code State-Action} pair
 * will lead to the same next {@code State}. In a grid world, moving UP from one square will always
 * lead to the square above it, so a learning rate of 1 would be correct.
 * <p>
